...
- [ ] Install Playwright in `frontend` directory: `pnpm create playwright`.
...
- [ ] Review existing `*.spec.ts` files in Backend.
- [ ] Ensure `AuthService` has unit tests for:
    - `validateUser` (success/failure).
    - `login` (returns JWT).
- [ ] Ensure `TasksService` has unit tests for:
...
- [ ] Unit Test Report for Backend (`pnpm test`).
