...
export const TaskItem: React.FC<TaskItemProps> = ({ task }) => {
  ...
  return (
    <div
      className={`bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex items-start justify-between group ${task.status === TaskStatus.DONE ? "opacity-60" : ""}`}
    >
      <div className="flex items-start space-x-3 flex-1">
        ...
        <div className="flex-1 min-w-0 ml-2">
          <div className="flex items-center justify-between">
            <h3
              className={`text-sm font-medium text-gray-900 ${task.status === TaskStatus.DONE ? "line-through text-gray-500" : ""}`}
            >
              {task.title}
            </h3>
            {project && (
